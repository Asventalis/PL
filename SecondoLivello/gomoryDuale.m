...
%   possibili seguendo le regole dell'algoritmo. ATagli e bTagli sono tali
%   per cui i piani di taglio corrispondono a ATagli*x<=bTagli.
%   PARAMETRI
...
%   info.equazioni
    ...
    if isnan(res.val) || isinf(res.val)
        ATagli = [];
        bTagli = [];
        info = [];
        return;
